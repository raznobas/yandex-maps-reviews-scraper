...
**Важно про блокировки (IP-адреса и прокси):**
Яндекс активно блокирует автоматические запросы из дата-центров (AWS, DigitalOcean, Hetzнер и т.д.). Если вы запускаете проект **локально** (с домашнего или офисного IP), скрапер обычно работает стабильно без прокси. 

Однако при деплое на **сервер** вы, скорее всего, столкнетесь с тем, что Яндекс будет отдавать пустые ответы при пагинации (загрузится только первая страница на 50 отзывов). Чтобы этого избежать, на сервере необходимо использовать **резидентские прокси (Residential Proxies)** из РФ или СНГ.
Настроить их можно, передав переменные окружения `HTTP_PROXY`, `HTTPS_PROXY` и `ALL_PROXY` в контейнеры `app` и `queue` в файле `docker-compose.yml`.
